...
If the existing application intentionally sends only selected/current updates, preserve that exact behavior. Migration must not silently change update-chain semantics.
...
Only one active transfer in parity release.
